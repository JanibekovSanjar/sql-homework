...
from(
	select *,
	StepNumber-row_number() over(partition by [Status] order by Stepnumber) as x
	from Groupings) as t
group by x,[Status]
...
WITH AllYears AS (
    SELECT TOP (YEAR(GETDATE()) - 1975 + 1)
        1975 + ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) - 1 AS HireYear
...
MissingYears AS (
    SELECT a.HireYear
    FROM AllYears a
    LEFT JOIN HireYears h ON a.HireYear = h.HireYear
